...
// Fetches product.html from the asset pipeline and rewrites its
// meta tags with real product data, then serves it to the client.
// The page JS (product.js) reads ?id= from the URL and loads the
// product normally — users see the full product detail page.
async function serveProductPage(id, context) {
  // Fetch product.html via the Cloudflare Pages asset binding and
  // the product data from Apps Script API simultaneously.
  const assetRequest = new Request(`${SITE_URL}/product.html`, context.request);

  const [assetRes, product] = await Promise.all([
    context.env.ASSETS
      ? context.env.ASSETS.fetch(assetRequest)
      : fetch(`${SITE_URL}/product.html`),
  ...
  return new HTMLRewriter()
    ...
    // Rewrite the product.js init so it reads the id from the clean URL
    // path segment rather than the ?id= query param.
    .on("body", {
      element(el) {
        el.append(
...
export async function onRequest(context) {
  ...
  if (!id) {
    return Response.redirect(`${SITE_URL}/shop`, 302);
  }

  return serveProductPage(id, context);
}
